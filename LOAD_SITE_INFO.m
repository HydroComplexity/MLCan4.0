...
CONSTANTS.Lf = 6000;                            % latent heat of fusion [J/mol]
...
PARAMS.retainLW = 1;                                                  % Factor to compute LW, what proportion of LW radiation emmited by a canopy
...
PARAMS.Soil.TK_litter = 0.15;%2;%Mere 0.15;                                         % Litter Thermal Conductivity [W/m/k] or [J/s/m/k]
PARAMS.Soil.TD_litter = 5.7*10^(-7);   %Mere 5.7*10^(-7);                                % Litter thermal diffusivity  [m/s]
PARAMS.Soil.slfactor = 0.1; %Mere 0.1                                  % Percentage above which is considered as snow for energy balance
...
ustarzeros = find(ustar_in == 0);
ustar_in(ustarzeros) = 0.001;
% if (SWITCHES.useG_on)
%     G_in = G_crop(inds);
% end
...
%Mere for inverse similarity, add:
ustar_in(binds) = vonk.*U_in(binds)./(log((2.5-((2/3)*.3))/0.125));
ustar_shrub=ustar_in;
ustar_shrub(binds)= vonk.*U_in(binds)./(log((2.5-((2/3)*.3))/0.125));
U_in_tree_1 = U_in + (ustar_shrub/vonk).*log(36/2.5);
avg_logs=((log((hcan-d0)/z0))*(.9))+(log((2.5-(2/3))/0.125)*(0.1));
ustar_tree = vonk.*U_in./(log((hcan-d0)/z0));
ustar_avg = vonk.*U_in./avg_logs;
U_in = U_in + (ustar_avg./vonk).*log(36/2.5);
% U_in=U_in+(ustar_shrub/vonk).*log((36-d0)/z0); error
% 
% remove:
...
%Mere - for similarity adjustment to top canopy layer
% h_toplayer=znc(20); %Mere
% d0_new=2/3*h_toplayer; %Mere
% ustar_in(binds) = vonk.*U_in(binds)./(log((hobs-d0_new)/z0));
% 
% % Adjust U from measurement height to canopy height (Brutsaert, 4.2)
% U_in = U_in - (ustar_in/vonk).*log(hobs/h_toplayer);
% uinds = find(U_in<.1);
% U_in(uinds) = .1;
...
%Mere edit - Make sure hcan & LAD max height are similar
if (abs(hcan-znc(20)) > 1)
    disp('NOTE! hcan is not equal to LAD canopy heights. Could cause inaccuracies!');
%     msg = 'ERROR! hcan is not equal to LAD canopy heights. Fix and rerun.';
%     error(msg)
end