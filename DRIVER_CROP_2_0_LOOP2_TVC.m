...
% Last updated: Meredith Richardson, 8/28/20
...
for Y = 1:numyearstoloop
    ...
    forcinglocation = 'D:\Users\Meredith\From Lyra\MLCan_MeredithEdits_working\MLCan4.0\Data\Arctic\TVCarctic\3yr_run_forcing_nospin\'; % add your file location here
    forcingfile = 'TVC_3yr_Forcing_'; % the forcing files should have a naming convention that ends in a number corresponding to the order you want to call them in (i.e. 1, 2, 3,...,65,..., etc.)
    sitestr = '.TVC_';
    sitestr2 = 'TVC_';
%     mtemp.fullpath_forcings = strcat(forcinglocation,forcingfile,num2str(Y));
    ...
    %     mtemp = matfile(tempfilename, 'Writable', true); %N
        ...
        forcinglocationshort = '.\Data\Arctic\TVCarctic\3yr_run_forcing_nospin\';
...
SWITCHES.arcticpeat = 1;          %  Arctic soil conditions, permafrost, peated soils
SWITCHES.coldregion = 1;          %  Turn off photosynthesis with frozen soil/deep snow
PARAMS.Photosyn.ph_minT = -7; %-7
PARAMS.Photosyn.ph_restartT = 3; %3
...
PARAMS.refl_PAR_can_snow = 0.2; %.2
PARAMS.refl_NIR_can_snow = 0.25;%0.275; %.275%.5  95-mixforest or trans wood/shrub,mineral land;110-mixfor,peat;150-conif,peat
...
PARAMS.inv_hobs = 4; PARAMS.inv_hcan = 0.3; %[m]
...
%         '_IC_NR1_', num2str(Y),'.mat'];
    namefilein = ['./Result' '/Result_','IC_', num2str(Y),'_Arctic.mat'];
end
...
namefileout2 = ['./Result/Treeline Analysis/Arctic/', sitestr2, num2str(Y+2015), '.mat'];
