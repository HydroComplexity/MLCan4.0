...
if (Rg>0 && ~isempty(fdiff) && entropymethod == 1)  % Landsberg's Method
    ...
    SScandif_out=(4/3).*(SWcandif_out/To).*Xout;        %Mere was SW_candif_in
...
% Diffuse in 
    ...
    if (Rg>0 && ~isempty(fdiff) && entropymethod == 1)  % Landsberg's Method
        ...
        Xout=0.9652+0.2777*log(1./eeout)+0.0511*eeout;            
        SSsoildif_out=(4/3).*(SWsoildif_out/To).*Xout;      %Mere was SW_soildif_in
...
if (Rg>0 && ~isempty(fdiff) && entropymethod == 1)  % Landsberg's Method
    ...
    Xout=0.9652+0.2777*log(1./eeout)+0.0511*eeout;            
    SSnetdif_out=(4/3).*(SWnetdif_out/To).*Xout; %Mere, was SWnetdif_in       
