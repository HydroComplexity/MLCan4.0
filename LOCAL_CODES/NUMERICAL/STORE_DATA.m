...
% Edited by Meredith Richardson, 2018
...
% LWemit_can_store(1,tt) = sum(LWabs_can);  %Mere:Incorrect; this was what it was doing before    %[W / m^2 ground] total absorbed LW by canopy  
LWabs_can_store(1,tt) = sum(LWabs_can);                                    %[W / m^2 ground] total absorbed LW by canopy
LWemit_can_store(1,tt) = sum(LWemit_can);                                  %[W / m^2 ground] total emitted LW by canopy  
LWemit_soil_store(1,tt) = LWemit_soil;                                     %[W / m^2 ground] emitted LW by soil
LWabs_soil_store(1,tt) = LWabs_soil;                                       %[W / m^2 ground] absorbed LW by soil
        
SWout_store(1,tt) = SWout;                                                 %[W / m^2 ground] total reflected SW (%Mere: incorrect "from canopy")
LWout_store(1,tt) = LWout;                                                 %[W / m^2 ground] total leaving LW (%Mere: incorrect "from canopy")
LWoutM_store(:,tt) = LWoutM;                                               %[W / m^2 ground] Vector of leaving LW from with information from LW emmited from different layers and atmosphere
...
refl_soil_store(1,tt) = VARIABLES.refl_soil;                               %[] Fraction of Reflected SW radiation by Soil 
        
% FLUXES:       
% Ecosystem Fluxes (Canopy + Soil)    
Fc_eco_store(1,tt) = Fc_soil - An_can;                                     %[umol CO2/ m^2 ground / s] flux of CO2 from surface, similar to NEE 
...
%Mere Additional Canopy Fluxes
for i=1:size(fLAIz,2)
    LWemit_can_all_store(:,tt,i)=VARIABLES.CANOPY.LWemit_can.*fLAIz(:,i);   %[W/m^2] Total LW leaving canopy for each species, all layers
    LWabs_can_all_store(:,tt,i)=VARIABLES.CANOPY.LWabs_can.*fLAIz(:,i);     %[W/m^2] Total LW absorbed by canopy for each species, all layers
    SWin_can_all_store(:,tt,i)=(VARIABLES.CANOPY.SWcandir_in + ...
        VARIABLES.CANOPY.SWcandif_in).*fLAIz(:,i);                          %[W/m^2] Total incoming SW into canopy for each species, all layers
    SWout_can_all_store(:,tt,i)=(VARIABLES.CANOPY.SWcandir_out + ...
        VARIABLES.CANOPY.SWcandif_out).*fLAIz(:,i);                         %[W/m^2] Total SW leaving canopy for each species, all layers
end
...
Tl_sun_Ta_Diff(:,tt,:) = Tl_sun - repmat(TAz,1,nspecies);                  %[C] Temperature Difference Atmosphere and Leaf Sunlit for all layers and species
...
Tl_sun_weighted(:,tt) = (nansum(Tl_sun.*fLAIz,2));                         %[C] Leaf Temperature Sunlit Fraction weighted by species for all layers
Tl_shade_weighted(:,tt) = (nansum(Tl_shade.*fLAIz,2));                     %[C] Leaf Temperature Shaded Fraction weighted by species for all layers
...
% Observed Canopy Variables
TAz_obs(tt)=FORCING.Ta;                                                    % Observed Air Temp [C]
CAz_obs(tt)=FORCING.Ca;                                                    % Observed CO2 Concentration in Air [umol/mol]
EAz_obs(tt)=FORCING.ea;                                                    % Observed Vapor Pressure in Air [kPa]
Uz_obs(tt)=FORCING.U;                                                      % Observed Wind Speed [m/s]
...
SimTa(1,tt) = VARIABLES.CANOPY.SimTa;                                      %[C] Air Temp at Canopy Height after Similarity Theory 
...
LE_soil_store(1,tt) = VARIABLES.SOIL.LEs;                                  %[W/m2] Latent Heat from the soil only  
LE_lit_store(1,tt) = VARIABLES.SOIL.LEl;                                  %[W/m2] Latent Heat from snow litter pack  
...
if (SWITCHES.entropy_on)
  ...
  H_net_store(1,tt) = SSresults.H_net;                                     % [W/m^2] Net (All Ecosystem) Energy dissipated H
