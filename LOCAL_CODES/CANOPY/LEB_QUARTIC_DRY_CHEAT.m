function [Tl, H, LE, dHcan, gv] = LEB_QUARTIC_DRY_CHEAT (SWITCHES, ...
...
%
%   Rabs = LE + H + Me + LWout + dHcan
%
%   dHcan is the storage in the leaves during this time step
%
...
%*************************************************************************
    ...
    if (sunlit)
        ...
        Tlprev = VARIABLES.CANOPY.Tl_prev_dt;                              % [C] Leaf temperature in previous time step       
    else
    ...
    VAratio = PARAMS.CanStruc.VAratio;                                     % [] Ratio of volume to area in the leaves
    cpl_JkgK = CONSTANTS.cpl_JkgK;                                         % [] specific heat capacity of leaves[J/kg/K]
    rho_leaf = CONSTANTS.rho_leaf;                                         % [] Density of Leaves [kg / m^3]
    ...
    nvinds = sort(unique([nvinds1 ; nvinds2]));                            % [] Final Indicator 
...
% Total vapor conductance (gv)
    ...
    if LWmethod == 0
        ...
        if isinf(bb)>0
            bb=0;
            fprintf('Infinite bb from LAIfrac=0')
        end
    else
    ...
    T1 = ee.*c4 + dd + 4*bb.*aa.^3 + VAratio*cpl_JkgK*rho_leaf/dtime;       % * Tl
    T0 = ee.*(c5-eaz) - dd.*Taz + bb.*aa.^4 - Rabs + Me - Tlprev*VAratio*cpl_JkgK*rho_leaf/dtime;                    % constant term
...
%   Initialize var vector with zeros, later we select  max(var);
    ...
    % Error Check
    if (rcnt == 0)
        disp(['NO REAL, POSITIVE ROOTS FOUND IN LEB DRY!!!'])
       % keyboard;
        Tl(ii) = Taz(ii);
    else    
        Tl(ii) = Tl_all;
...
dHcan = (Tl-Tlprev)*VAratio*cpl_JkgK*rho_leaf/dtime;
