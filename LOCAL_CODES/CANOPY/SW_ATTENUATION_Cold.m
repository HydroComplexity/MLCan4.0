    ...
    for ii = tind:-1:bind
      ...
      % BEAM RADIATION     
        if (Kbm>0 && beam_top>0 && count==0)
            ...
            if (ii<tind)
                diffup(ii+1) = refl_layers(ii)*beam_int;
            else
                reflbup = refl_layers(ii)*beam_int; % used for debugging
            end
        else
            reflbup = 0;
        end
      ...
      % DIFFUSE RADIATION
        ...
        % upward reflection
        if (ii<tind)
            diffup(ii+1) = diffup(ii+1) + refl_layers(ii)*diff_int;
        else
            refldup = refl_layers(ii)*diff_int; % for debugging
        end

        % absorbed fraction
        sun_abs(ii) = sun_abs(ii) + (1-refl_layers(ii)-trans)*diff_int*fsun(ii);
        shade_abs(ii) = shade_abs(ii) + (1-refl_layers(ii)-trans)*diff_int*fshade(ii);
        
        % ******************* to compute entropy *********************    
        candif_in(ii) = candif_in(ii) + diff_int;
        candif_out(ii) = candif_out(ii) + diff_int*(refl_layers(ii)+trans); 
        % ******************* to compute entropy *********************    
...
%==========================
    for ii = bind:tind
        ...
        % downward reflection
        if (ii==bind)
            soil_inc = soil_inc + refl_layers(ii)*diff_int;
            soildif_inc = soildif_inc + refl_layers(ii)*diff_int;            
        else
            diffdn(ii-1) = diffdn(ii-1) + refl_layers(ii)*diff_int;
        end
