function [Ph, An, Ci, gsv, Tl, LE, dHcan, TR, Evap_mm, H, psil, fsvg, fsvm, Ch2o_mm, gbv, gbh, ...
          ...
          VARIABLES,converged] = ...
    LEAF_SOLUTION(FORCING, VARIABLES, PARAMS, CONSTANTS, VERTSTRUC, SWITCHES, sunlit, cntspecies, tt)
...
%*************************************************************************
    ...
    % CONSTANTS
    if (SWITCHES.Lv_T)
        Lv_kg = 1.91846e06.*(Ta_K./(Ta_K - 33.91)).^2;                        % [J /kg]
    else
        Lv_kg = CONSTANTS.Lv_kg;   
    end
    ...
    if (sunlit)  
        Tl_prev =  VARIABLES.CANOPY.Tl_sun(:,cntspecies);                    %Mere uncommented
        ...
        Tl_prev =  VARIABLES.CANOPY.Tl_shade(:,cntspecies);                  %Mere uncommented
...
relax = 0;
relaxval = 0.25;
osc = 0.01; %Mere This is the range for which lack of oscillation turns relax off 
maxchange = 0.25;
maxiters = 10;
converged = 0;  cnt = 0;
conv_perc = 0.01; %Mere percent difference for convergence
...
while(~converged)
    ...
    % PHOTOSYNTHESIS
        if (ph_type(cntspecies)==1) 
            % C3
            [Ph, An, Ph_limit, Jc_C3, Jj_C3, Js_C3, photo] = PHOTOSYNTHESIS_C3(VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, SWITCHES, sunlit,cntspecies);
    ...
    % LEAF ENERGY BALANCE - DRY LEAF FRACTION
        if (canstorheat)
            [Tl_dry, H_dry, LE_dry, dHcan_dry, gv_dry] = LEB_QUARTIC_DRY_CHEAT (SWITCHES,...
                VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
        else
            [Tl_dry, H_dry, LE_dry, gv_dry] = LEB_QUARTIC_DRY (SWITCHES,...
                VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
            dHcan_dry = nan;            
        end
    ...
    %   H_wet is alway zero
        if (canstorheat)    
            [Tl_wet, H_wet, LE_wet, dHcan_wet, gv_wet, vindswet, nvindswet] = LEB_QUARTIC_WET_CHEAT (SWITCHES,...
                VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
        else
            [Tl_wet, H_wet, LE_wet, gv_wet, vindswet, nvindswet] = LEB_QUARTIC_WET (SWITCHES,...
                VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
            dHcan_wet = nan;
        end            
    ...
    if (cnt > 0)
        ...
        if isempty (vinds)
            ...
            dcm = max(abs((Ci(vinds)-Ci_prev(vinds))./Ci_prev(vinds)));
            dgm = max(abs((gsv(vinds)-gsv_prev(vinds))./gsv_prev(vinds)));
            dtm = max(abs((Tl(vinds)-Tl_prev(vinds))./Tl_prev(vinds)));
            
            if ( dcm > maxchange || ...
                 dgm > maxchange || ...
                 dtm > maxchange )

%                 disp(['*** Ci Divergence --> Timestep: ',num2str(tt)]) %Mere: display to diagnose divergence
...
%                 Tl = Tl_prev;
        ...
        if (cnt>2)
           ...
           if ( (md1 > (md2-osc*md2)) && (md1 < (md2+osc*md2)) && relax > 0 ) %Mere added relax constraint, otherwise loop is pointless
               relax = 0;
           end
        end
    ...
    % TEST CONVERGENCE
        if (cnt>0 && relax == 0)
            gm = max(abs((gsv-gsv_prev)./gsv_prev));
            cm = max(abs((Ci-Ci_prev)./Ci_prev));
            tm = max(abs((Tl-Tl_prev)./Tl_prev));
            if nansum(Tl) == 0
                if ( (gm < conv_perc) && ...
                    (cm < conv_perc) )
                    converged = 1;
...
%             disp(['*** TOO MANY INTERATIONS IN LEAF MODEL!!! --> Timestep: ',num2str(tt)])
            break;
        end
