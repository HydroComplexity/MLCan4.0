...
% For Cold Regions
coldregion = SWITCHES.coldregion;
zsn = VARIABLES.SOIL.zsn/1000;                                             % [m] depth of snow
hcan = PARAMS.CanStruc.hcan;                                               % [m] canopy height
zhc = VERTSTRUC.zhc;                                                       % [m] top of each canopy layer;for mid-layer use znc
refl_PAR_layers = refl_PAR.*ones(size(zhc));
refl_NIR_layers = refl_NIR.*ones(size(zhc));
refl_PAR_can_snow = PARAMS.refl_PAR_can_snow;
refl_NIR_can_snow = PARAMS.refl_NIR_can_snow;
%*************************************************************************
%*************************************************************************
...
% Diffuse Fraction
    ...
    if (coldregion)
        if (zsn > zhc(1)) %top of each layer; mid-layer use znc
            layers_under_snow = find(zsn > zhc);
%             refl_PAR_layers(layers_under_snow) = refl_PAR_layers(layers_under_snow)...
%                 + refl_PAR_layers(layers_under_snow).*PARAMS.lc_refl_change./100; %Perc change in reflectance from snow
%             refl_NIR_layers(layers_under_snow) = refl_NIR_layers(layers_under_snow)...
%                 + refl_NIR_layers(layers_under_snow).*PARAMS.lc_refl_change./100;
            refl_PAR_layers(layers_under_snow) = max((refl_PAR_can_snow - (snow_tcount/48/12.5)*refl_snow_old),0.1);%refl_PAR
            refl_NIR_layers(layers_under_snow) = max((refl_NIR_can_snow - (snow_tcount/48/12.5)*refl_snow_old),0.1);%refl_NIR
        end
    end
...
if (PARtop<1)
    ...
    while (percdiff > 0.01)
        ...
        [PARabs_sun, PARabs_shade, PARcandir_in, PARcandir_out, PARcandif_in, PARcandif_out,...
            ...
            SW_ATTENUATION_Cold (beam_top, diff_top, LAIz, ...
                            ...
                            fsun, fshade, radlost, refl_PAR_layers);
        else
        [PARabs_sun, PARabs_shade, PARcandir_in, PARcandir_out, PARcandif_in, PARcandif_out,...
            PARabs_soil, PARsoildir_in, PARsoildir_out, PARsoildif_in, PARsoildif_out, fsun, fshade, diffdn, diffup, ...
                radabs_tot, radlost, PARradremain] = ...
    ...
    while (percdiff > 0.01)
        ...
        [NIRabs_sun, NIRabs_shade, NIRcandir_in, NIRcandir_out, NIRcandif_in, NIRcandif_out,...
            ...
            SW_ATTENUATION_Cold (beam_top, diff_top, LAIz, ...
                            ...
                            fsun, fshade, radlost, refl_NIR_layers);
        else
