...
compflux(psicom,psiroot,kf,krad,ft,fb,dzsoi,zsoi,nl_soil, type,thetai,...
thetaf,CC_Ss, psii,dtime,nspecies,VARIABLES,SWITCHES, theta_dry)
...
% No water uptake from dry soil - %Mere 
flux_sr(thetaf<=theta_dry) = 0;

if (SWITCHES.coldregion) %Mere frozen soil = no uptake
    frozenlayers = find(VARIABLES.SOIL.Ts <-1);
    flux_sr(frozenlayers) = 0;
    flux_sr_all(frozenlayers,:) = 0;
end
...
if abs(mberror) > .01
    stop=10
    fprintf('Frozen Soil Uptake mass balance error %d \n',mberror)
end
% Dongkook: End
