function [remain, Hl, LEl, G1, RH, ra, psil_MPa, psil, LWups] = SEB_Remainder_litter(Tli, Tsl, Tlprev, VHC_SL, dtime,...
                                   ...
                                   rho_dry_air, cp, Lv, Lf, boltz, Vw, R, mmH2OtoMPa, thetamin, case1sl, G_obs)
...
%     if Tli<Ta1
%         e1 = 0.611*exp(17.502*Ta1/(Ta1+240.97)); %Mere
%     else
        e1 = ea1;
%     end
    %compute LE
    if case1sl 
        LEl = (Ls.*rhoa*(0.622/pa)*(esatTl - e1))/(rl+ra);   %ea1 %Mere temp
    ...
    G1 = (TK_sl)*(Tli - Tsl)/((zsl)/2); %Mere G_obs;
