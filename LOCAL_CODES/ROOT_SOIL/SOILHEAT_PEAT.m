function [VARIABLES] = SOILHEAT (Hg, VARIABLES, VERTSTRUC, PARAMS, CONSTANTS, SWITCHES)   
...
%   dt   dz        dz  
%   
%  Written By: Darren Drewry and Modified by Juan Quijano and edited for 
%  Arctic peat by Meredith Richardson
%
%------------------------- Input Variables -------------------------------
...
%         %ke = ones(12,1)*0.8;
    ...
    % Soil Thermal Conductivity [W / m / K] [Zhao et al. 2019; doi=10.1016/j.agrformet.2019.04.004]
    %   TK_soil = A+Bln(1+Sr)
        m  = 0.6; % soil-texture parameter
        Ke = m.*Sr./(1+(m-1).*Sr);
        %Tk sat calculated correctly above
        A = TK_dry; 
        B = (TK_sat - TK_dry)./log(2);
        TKsoil = A+B.*log(1+Sr);
