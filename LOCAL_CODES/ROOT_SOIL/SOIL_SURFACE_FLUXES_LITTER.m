...
%*************************************************************************
    ...
    if(SWITCHES.Lv_T)
        Ta_K = 273.15 + VARIABLES.CANOPY.TAz(1);
    ...
    Ts1 = VARIABLES.SOIL.Ts(1);                                             % [C] Temperature in the Soil first layer 
    ...
    Tlprev = VARIABLES.SOIL.Tlprev;                                         % [C] Previous Soil Temperature
    ...
    Tf = PARAMS.Soil.Tf;                                                    % [K] Freezing Temperatures of fresh water
    ...
    %G_obs = PARAMS.G_obs;                                                   %Mere

    dzs1 = dzs1(1);        
    psis1_MPa = psis1 * mmH2OtoMPa;   
    
    if(VARIABLES.timestep>240)
        Tsl;
    end
%*************************************************************************
...
% iii. Calculate the variable of both cases.
    if zicesl >= slfactor*dzlit_mm*thetals && Tsl <= Tf
        case1sl = true;                             % Ice   
    else
...
while abs(dif) > 0.005
       ...
       dH = 0;                                           
        ...
        if littertype == 1
             ...
             if case1sl == 1
                       ...
                       if abs(Gs) > 4*abs(Rabs)
                               ...
                               [remain2, LEs, Gsl, Gs, Tsl] = SEB_Remainder_soil3(Tli, Ts1,...
                                           zsl_m, TK_sl, dzs1, TC1, Af2*Rabs);    
                                       %did call SEB_Remainder_soil3_correction, but does not exist
                                       %added input: Gscorr 
                       end   
             ...
             elseif case1sl == 0  
                ...
                [remain2, LEs, Gsl, Gs] = SEB_Remainder_soil1(Tli, Tsl, Ts1, ea1, pa1, U1, z1,...
                                       vonk, z0, zsl_m, TK_sl,...
                                       rho_dry_air, psis1_MPa, dzs1, TC1, Af2*Rabs);%, G_obs);%Mere );                                    
             end                                   
        ...
        elseif littertype == 2
            ...
            if case1sl == 1
                       [remain2, LEs, Gsl, Gs, Tsl] = SEB_Remainder_soil3(Tli, Ts1,...
                                   zsl_m, TK_sl, dzs1, TC1, Af2*Rabs);%, G_obs);%Mere );                  
            else                
                        ...
                        [remain2, LEs, Gsl, Gs] = SEB_Remainder_soil2(Tl, Tsl, Ts1, porsl1,...
                                   pa1, c1, thetatr, psill, bl,...
                                   bdl, rhowater, zsl_m, TK_litter, ldif, volliqli,...
                                   rho_dry_air, mmH2OtoMPa, psis1_MPa, dzs1, TC1, Af2*Rabs);%, G_obs);%Mere );                               
            end            
        end   
