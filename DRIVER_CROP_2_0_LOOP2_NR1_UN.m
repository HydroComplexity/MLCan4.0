...
for Y = 1:numyearstoloop
    ...
    forcinglocation = 'D:\Users\Meredith\From Lyra\MLCan_MeredithEdits_working\MLCan4.0\Data\Niwot Ridge\NR1_subalpine\6yr_run_forcing_scaled\'; % add your file location here
    forcingfile = 'NR1_6yr_Forcing_'; % the forcing files should have a naming convention that ends in a number corresponding to the order you want to call them in (i.e. 1, 2, 3,...,65,..., etc.)
    sitestr = '.NR1_un_';
    sitestr2 = 'NR1_un_';
%     mtemp.fullpath_forcings = strcat(forcinglocation,forcingfile,num2str(Y));
    ...
    %     mtemp = matfile(tempfilename, 'Writable', true); %N
        ...
        forcinglocationshort = '.\Data\Niwot Ridge\NR1_subalpine\6yr_run_forcing_scaled\';
...
PARAMS.Photosyn.ph_minT = -3;     % site-specific - Bowling et al 2018, DOI: 10.1016/j.agrformet.2018.01.025
PARAMS.Photosyn.ph_restartT = 5;
PARAMS.photottlimit = [0, 18000, 18000, 0]; %[5000, 14000, 8500, 12000]; % photo hibernation limits [restart min, restart max, stop max, stop min]
% default: [0, 18000, 18000, 0]
PARAMS.refl_PAR_can_snow = 0.1;
PARAMS.refl_NIR_can_snow = 0.07;%0.2;        %  95-mixforest or trans wood/shrub,mineral land;110-mixfor,peat;150-conif,peat
...
SWITCHES.replacing_on = 0;
SWITCHES.invsimilarity = 2;
PARAMS.inv_hobs = 36; PARAMS.inv_hcan = 1.2; %[m]
PARAMS.true_hcan = PARAMS.inv_hcan;
...
namefileout2 = ['./Result/Treeline Analysis/NWR/', sitestr2, num2str(Y+2007), '.mat'];
